...
def clear_all_cache():
    """Flush all cache layers: Upstash REST, redis-py, and in-memory."""
    ...
    # 2. Clear Upstash REST
    url, token = _get_upstash_rest()
    if url:
        try:
            import urllib.request
            body = json.dumps(["FLUSHDB"]).encode()
            req = urllib.request.Request(
                url,
                data=body,
                headers={
            ...
            with urllib.request.urlopen(req, timeout=5) as resp:
                resp.read()
            cleared.append("upstash")
        except Exception as exc:
            logger.warning("[cache] Upstash FLUSHDB failed: %s", exc)
    ...
    if r is not None:
        try:
            ...
            logger.warning("[cache] redis-py FLUSHDB failed: %s", exc)

    logger.info("[cache] 🗑️ Cache cleared: %s", ", ".join(cleared))
    return cleared
